...
def difficult_lvl():
    ...
    # лучше учесть несколько вариантов ответа
...
def load_game():
    ...
    if PLAYER in SAVES:
        ...
        if choice in ('yes', 'y', 'да', 'д'):
            # Загрузка игры
            pass
        # а для всех прочих в данном случае можно реализовать вариант "нет"
        else:
            pass
    ...
    if current:
        for p in PLAYER:
            if not p.startswith('ai'):
                print(f"Статистика побед, поражений и ничьих для - {p} - {PLAYERS[p]}")
